...
class ConcurrencyCap(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        """Admit, reject, or bypass one request.

        Contract (hidden tests grade exactly this observable behavior):
        - Requests whose path is in _BYPASS_PATHS are passed straight through,
          never counted against the cap.
        - Over-cap requests are rejected IMMEDIATELY (no queueing) with
          status 503, JSON body {"error": {"type": "rate_limit", "message": ...}},
          and headers Retry-After (RETRY_AFTER as a string),
          X-Concurrency-In-Flight, and X-Concurrency-Max.
        - Admitted requests are forwarded; their slot is released even if the
          downstream app raises; successful responses carry the
          X-Concurrency-In-Flight and X-Concurrency-Max headers.
        """
        raise NotImplementedError("excised for harness-ratchet task 09")
