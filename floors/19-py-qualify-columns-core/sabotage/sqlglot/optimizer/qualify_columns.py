...
from sqlglot.optimizer.scope import Scope, build_scope, find_all_in_scope, find_in_scope, traverse_scope, walk_in_scope
...
def qualify_columns(expression: E, schema: dict[str, object] | Schema, expand_alias_refs: bool=True, expand_stars: bool=True, infer_schema: bool | None=None, allow_partial_qualification: bool=False, dialect: DialectType=None) -> E:
    """
    ...
    for scope in traverse_scope(expression):
        ...
        if (schema.empty or dialect.FORCE_EARLY_ALIAS_REF_EXPANSION) and expand_alias_refs:
            _expand_alias_refs(scope, resolver, dialect, expand_only_groupby=dialect.EXPAND_ONLY_GROUP_ALIAS_REF)
...
def _separate_pseudocolumns(scope: Scope, pseudocolumns: set[str]) -> None:
    ...
    for column in scope.columns:
        ...
        if name not in pseudocolumns:
            continue
        if name != 'LEVEL' or (isinstance(scope_expression, exp.Select) and scope_expression.args.get('connect')):
    ...
    for i, join in enumerate(joins):
        ...
        if using is None and join.method == 'NATURAL':
            if columns and '*' not in columns and join_columns and ('*' not in join_columns):
                using = [exp.to_identifier(column_name) for column_name in columns if column_name in join_columns]
                if using:
...
def _expand_alias_refs(scope: Scope, resolver: Resolver, dialect: Dialect, expand_only_groupby: bool=False) -> None:
    ...
    def replace_columns(node: exp.Expr | None, resolve_table: bool=False, literal_index: bool=False) -> None:
        nonlocal replaced
        ...
        for column in walk_in_scope(node, prune=lambda node: node.is_star):
            ...
            if alias_expr:
                skip_replace = bool(find_in_scope(alias_expr, exp.AggFunc) and (is_group_by or (column.find_ancestor(exp.AggFunc) and (not isinstance(column.find_ancestor(exp.Window, exp.Select), exp.Window)))))
            ...
            if table and (not alias_expr or skip_replace):
                ...
                if (isinstance(alias_expr, exp.Literal) or alias_expr.is_number) and (literal_index or resolve_table):
                    if literal_index:
                    ...
                    if resolve_table and resolver.schema.empty:
                        for inner in walk_in_scope(column):
                            if isinstance(inner, exp.Column) and (not inner.table) and (inner_table := resolver.get_table(inner)):
    ...
    while parent_scope and (not parent_scope.is_cte):
        ...
        if (parent_scope := parent_scope.parent):
            if isinstance(parent_scope.expression, exp.Union):
                on_right_sub_tree = parent_scope.expression.expression.unnest() is child_scope.expression
    ...
    if dialect.PROJECTION_ALIASES_SHADOW_SOURCE_NAMES:
        for clause in (expression.args.get('group'), expression.args.get('having'), expression.args.get('qualify')):
            if not clause:
                continue
...
def _expand_order_by_and_distinct_on(scope: Scope, resolver: Resolver) -> None:
    ...
    for modifier_key in ('order', 'distinct'):
        ...
        for original, expanded in zip(modifier_expressions, _expand_positional_references(scope, modifier_expressions, resolver.dialect, alias=True)):
        ...
        if expr.args.get('group'):
            ...
            for node in modifier_expressions:
                node.replace(exp.to_identifier(_select_by_pos(expression, node).alias) if node.is_int else selects.get(node, node))

def _expand_positional_references(scope: Scope, expressions: Iterable[exp.Expr], dialect: Dialect, alias: bool=False) -> list[exp.Expr]:
            ...
            if alias:
                ...
                if dialect.PROJECTION_ALIASES_SHADOW_SOURCE_NAMES:
                    if ambiguous_projections is None:
                        ambiguous_projections = {s.alias_or_name for s in expression.selects if s.alias_or_name in scope.selected_sources}
                    ambiguous = any((column.parts[0].name in ambiguous_projections for column in select_expr.find_all(exp.Column)))
                else:
                    ambiguous = False
                if isinstance(select_expr, exp.CONSTANTS) or select_expr.is_number or select_expr.find(exp.Explode, exp.Unnest) or ambiguous:
...
def _convert_columns_to_dots(scope: Scope, resolver: Resolver) -> None:
    ...
    for column in itertools.chain(scope.columns, scope.stars):
        ...
        if column_table and column_table not in scope.selected_sources and (not scope.parent or column_table not in scope.parent.sources or (not scope.is_correlated_subquery)):
...
def _qualify_positional_column(scope: Scope, resolver: Resolver, column: exp.Column, column_table: str, column_source: exp.Expr | Scope, source_columns: t.Sequence[str], pivots: t.Sequence[exp.Pivot], allow_partial_qualification: bool) -> bool:
    """Resolve a positional column, returning whether to skip further qualification."""
    if not resolver.dialect.SUPPORTS_POSITIONAL_COLUMN_REFS or not isinstance(column.this, exp.Parameter) or (not isinstance((position := column.this.this), exp.Literal)) or (not position.is_int):
        return False
    scope_pivot = next((pivot for pivot in scope.pivots if pivot.alias == column_table), None)
    if not scope_pivot:
        scope_pivot = next((pivot for pivot in reversed(scope.pivots) if pivot.parent and pivot.parent.alias_or_name == column_table), None)
    ...
    if isinstance(column_source, exp.Table):
        ...
        source_columns_without_aliases = resolver.schema.column_names(column_source, only_visible=True)
        source_columns_incomplete = not source_columns_without_aliases or '*' in source_columns_without_aliases
    else:
        alias_columns = []
    ...
    if not 1 <= position_value <= len(positional_columns):
        if allow_partial_qualification:
            return True
        raise OptimizeError(f"Positional reference ${position_value} is out of range for source '{column_table}'")
    ...
    if isinstance(column_source, Scope):
        ...
        selection = source_selects[position_value - 1] if position_value <= len(source_selects) else None
...
def _qualify_columns(scope: Scope, resolver: Resolver, allow_partial_qualification: bool) -> None:
    ...
    for column in scope.columns:
        ...
        if column_table and column_table not in scope.sources:
            column_source = scope.sources[column_table]
            source_columns = resolver.get_source_columns(column_table)
            pivots = column_source.args.get('pivots', []) if isinstance(column_source, exp.Table) else []
            if pivots and source_columns and ('*' not in source_columns):
                for pivot in pivots:
                    source_columns = pivot.output_columns(source_columns)
            if _qualify_positional_column(scope, resolver, column, column_table, column_source, source_columns, pivots, allow_partial_qualification):
                continue
            column_name = column.name
        ...
        if not column_table:
            ...
            if table and isinstance((source := scope.sources.get(table.name)), Scope) and (id(column) in source.column_index):
                continue
            if table:
                column.set('table', table)
            elif resolver.dialect.TABLES_REFERENCEABLE_AS_COLUMNS and len(column.parts) == 1 and (column_name in scope.selected_sources):
    ...
    available: t.Collection[str] = resolver.get_source_columns(pivoted_source) if pivoted_source in scope.sources else []
    for pivot in pivots:
...
def _expand_struct_stars_no_parens(expression: exp.Dot) -> list[exp.Alias]:
    ...
    for field in t.cast(exp.DataType, starting_struct.kind).expressions:
        ...
        new_column = exp.column(t.cast(exp.Identifier, root), table=dot_column.args.get('table'), fields=t.cast(list[exp.Identifier], parts))
    ...
    while parent is not None:
        ...
        for struct_field_def in t.cast(exp.DataType, starting_struct).expressions:
            if struct_field_def.name == rhs.name:
                matched = True
                starting_struct = struct_field_def.kind
                break
        if not matched:
            return []
        parent = parent.parent
...
def _expand_stars(scope: Scope, resolver: Resolver, using_column_tables: dict[str, t.Any], pseudocolumns: set[str], annotator: TypeAnnotator) -> None:
    ...
    annotated_ahead = dialect.SUPPORTS_STRUCT_STAR_EXPANSION and any((isinstance(col, exp.Dot) for col in scope.stars))
    if annotated_ahead:
        annotator.annotate_scope(scope)
    scope_expression = scope.expression
    ...
    for expression in scope_expression.selects:
        ...
        for table in tables:
            ...
            if source is None:
                chain = scope.pivots
                parent = chain[-1].parent if chain and chain[0].parent is chain[-1].parent and (chain[-1].alias == table) else None
                if parent:
                    pivots = chain
            ...
            quoted_columns = {s.output_name for s in source_expression.selects if _is_output_identifier_quoted(s)} if isinstance(source_expression, exp.Query) else set()
            if pivots is None:
                ...
                pivots = selected_node.args.get('pivots') if isinstance(selected_node, exp.Expr) else None
            if pivots:
                pivot_columns: t.Collection[str] = columns
                for pivot in pivots:
                    pivot_columns = pivot.output_columns(pivot_columns) or pivot.alias_column_names
                if pivot_columns:
                    new_selections.extend((alias(exp.column(name, table=pivots[-1].alias or None), name, copy=False) for name in pivot_columns if name not in columns_to_exclude))
                    continue
            for name in columns:
                ...
                if name in using_column_tables and table in using_column_tables[name]:
                    ...
                    quoted = name in quoted_columns or (isinstance(source, exp.Table) and dialect.case_sensitive(name))
                    selection_expr = replaced_columns.get(name) or exp.column(name, table=table, quoted=quoted)
...
def _add_rename_columns(expression: exp.Expr, tables, rename_columns: dict[int, dict[str, str]]) -> None:
...
def _add_replace_columns(expression: exp.Expr, tables, replace_columns: dict[int, dict[str, exp.Alias]]) -> None:
...
def qualify_outputs(scope_or_expression: Scope | exp.Expr, dialect: Dialect) -> None:
    ...
    if not isinstance(expression, exp.Selectable):
        return
    new_selections = []
    for i, (selection, aliased_column) in enumerate(itertools.zip_longest(expression.selects, scope.outer_columns)):
