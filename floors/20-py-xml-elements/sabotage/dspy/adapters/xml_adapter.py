...
from dspy.adapters.utils import apply_output_field_defaults, format_field_value, parse_value, serialize_for_json, translate_field_type
...
class XMLAdapter(ChatAdapter):
    ...
    def format_field_structure(self, signature: type[Signature]) -> str:
        ...
        def format_field(name, field):
            if (field.json_schema_extra or {}).get('__dspy_field_type') == 'output' and self._uses_nested_xml(field.annotation):
                return self._xml_schema(name, field.annotation)
            return self.format_field_with_value({FieldInfoWithName(name=name, info=field): translate_field_type(name, field)})
    ...
    def parse(self, signature: type[Signature], completion: str) -> dict[str, Any]:
        ...
        for name, field in signature.output_fields.items():
            ...
            for candidate in [schema, *schema.get('anyOf', [])]:
                ...
                raise AdapterParseError(adapter_name='XMLAdapter', signature=signature, lm_response=completion, message=f'Failed to parse field {field} with value {value}: {error}') from error
    ...
    def _value_to_xml(cls, value: Any, tag: str, key: str | None=None) -> str:
        ...
        if isinstance(value, dict):
            ...
            for name, child in value.items():
                ...
                children.append(cls._value_to_xml(child, name) if valid_name else cls._value_to_xml(child, 'entry', name))
    ...
    def _schema_to_xml(cls, tag: str, schema: dict, definitions: dict, seen: frozenset[str]) -> str:
        ...
        if (choices := schema.get('anyOf')):
            return cls._schema_to_xml(tag, next((s for s in choices if s.get('type') != 'null'), choices[0]), definitions, seen)
        if schema.get('type') == 'array':
            item = cls._schema_to_xml('item', schema.get('items', {}), definitions, seen)
            return f'<{tag}>{item}</{tag}>'
        children = ''.join((cls._schema_to_xml(name, child, definitions, seen) for name, child in schema.get('properties', {}).items()))
    ...
    def _elements_to_value(cls, elements: list[ET.Element], schema: dict, definitions: dict) -> Any:
        ...
        if (choices := schema.get('anyOf')):
            if {'type': 'null'} not in choices and (not list(elements[0])) and (not (elements[0].text or '').strip()):
                return None
        ...
        if not isinstance(child_schema, dict):
            child_schema = {}
        return {name: cls._elements_to_value(items, properties.get(name, child_schema), definitions) for name, items in children.items()}
    ...
    def _uses_nested_xml(annotation: Any) -> bool:
        ...
        return not is_dspy_model and (origin in (list, dict) or is_typeddict(annotation) or (isinstance(annotation, type) and issubclass(annotation, pydantic.BaseModel)))
